...
def train(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        # Métricas por classe para treino
        train_f1_per_class = f1_score(train_labels, train_preds, average=None)
        train_precision_per_class = precision_score(train_labels, train_preds, average=None)
        train_recall_per_class = recall_score(train_labels, train_preds, average=None)
        ...
        for i, class_name in enumerate(class_names):
            print(f"    {class_name}: Precision={train_precision_per_class[i]:.4f}, Recall={train_recall_per_class[i]:.4f}, F1={train_f1_per_class[i]:.4f}")
        ...
        eval_precision = precision_score(eval_labels, eval_preds, average='macro')
        
        # Métricas por classe para validação
        eval_f1_per_class = f1_score(eval_labels, eval_preds, average=None)
        eval_precision_per_class = precision_score(eval_labels, eval_preds, average=None)
        eval_recall_per_class = recall_score(eval_labels, eval_preds, average=None)
        eval_cm = confusion_matrix(eval_labels, eval_preds)
        ...
        for i, class_name in enumerate(class_names):
            print(f"    {class_name}: Precision={eval_precision_per_class[i]:.4f}, Recall={eval_recall_per_class[i]:.4f}, F1={eval_f1_per_class[i]:.4f}")
        ...
        epoch_metrics = {
            f"epoch_{epoch}": {
                ...
                "training_avg_precision": float(train_precision),
                "training_f1_measure": float(train_f1),
                "training_loss": float(avg_train_loss),
                "training_confusion_matrix": train_cm.tolist(),
                "training_per_class_metrics": {
                    class_name: {
                        "precision": float(train_precision_per_class[i]),
                        "recall": float(train_recall_per_class[i]),
                        "f1_score": float(train_f1_per_class[i])
                    }
                    for i, class_name in enumerate(class_names)
                },
                "validation_accuracy": float(eval_accuracy),
                "validation_avg_recall": float(eval_recall),
                "validation_avg_precision": float(eval_precision),
                "validation_f1_measure": float(eval_f1),
                "validation_loss": float(avg_eval_loss),
                "validation_confusion_matrix": eval_cm.tolist(),
                "validation_per_class_metrics": {
                    class_name: {
                        "precision": float(eval_precision_per_class[i]),
                        "recall": float(eval_recall_per_class[i]),
                        "f1_score": float(eval_f1_per_class[i])
                    }
                    for i, class_name in enumerate(class_names)
                }
            }
    ...
    all_metrics.update({
        ...
        "best_validation_avg_precision": float(all_metrics[f"epoch_{best_epoch}"]["validation_avg_precision"]),
